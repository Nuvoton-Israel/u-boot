...
int spi_copy_header(int from, int to)
{
	...
	rc = spi_flash_read(flash, 0x0, flash->erase_size, buf);
	printf("SF: %zu bytes @ %#x Read: %s\n", (size_t)flash->erase_size,
		0x0, rc ? "ERROR" : "OK");
	if(rc)
		goto done;
...
void check_rSelfTestPass_status(struct npcm_gcr *gcr)
{
	int val;
	printf("> 11\n");
	...
	if (readl(&gcr->intcr2) & (1 << INTCR2_SELFTEST_PASSED)) {
		...
		val |= (1 << FLOCKR1_UPDATE_APPROVE);
		writel(val, &gcr->flockr1);
		printf("> set rUpdateApprove flag 1, FLOCKR1 = 0x%08x\n", readl(&gcr->flockr1));
		update_flash();
	} else {
		printf("> 12\n");
		val = readl(&gcr->intcr2);
...
void nist_boot(void)
{
	...
	if (image == 0)	{	//Run Active Bootblock from flash #0
		...
		if (memcmp((void *)(secondBootBlockHeaderPtr), startTag, START_TAG_SIZE)) {
			...
			check_rSelfTestPass_status(gcr);
		}
		else {
	...
	else if (image == 2) {		//Run Recovery Bootblock from flash #1
		printf("> 5\n");
